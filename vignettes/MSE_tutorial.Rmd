...
A nice thing about working with data in two dimensions is that we can visualize our data along with the estimated clusters. We can see that our true clusters are preserved in both the training set and the test set after negative binomial count splitting. Thus, we will be able to get reasonable estimates of the true clusters using the training set, and evaluate these estimates well using the test set. 

```{r}
p1 <- ggplot(data=NULL, aes(x=Xtrain.known[,1]+1, y=Xtrain.known[,2]+1, col=as.factor(clusters)))+
  geom_point()+
  scale_x_log10()+
  scale_y_log10()+
  theme_bw()+
...
p2 <- ggplot(data=NULL, aes(x=Xtest.known[,1]+1, y=Xtest.known[,2]+1, col=as.factor(clusters)))+
  geom_point()+
  scale_x_log10()+
  scale_y_log10()+
  theme_bw()+
...
We can see that, if we estimate $k=3$ clusters on the training set, the clusters look reasonable when applied to the test set. 
...
Suppose that we estimate $k=6$ clusters on the training set. We can see that our evaluation will be poor on the test set, because the extra clusters will be driven by random noise that will not exist in the test set. 

```{r}
estimated.clusters.6 <- kmeans(log(Xtrain.known+1), 6)$cluster

p3 <- ggplot(data=NULL, aes(x=Xtrain.known[,1]+1, y=Xtrain.known[,2]+1, col=as.factor(estimated.clusters.6)))+
  geom_point()+
  scale_x_log10()+
  scale_y_log10()+
  theme_bw()+
...
p4 <- ggplot(data=NULL, aes(x=Xtest.known[,1]+1, y=Xtest.known[,2]+1, col=as.factor(estimated.clusters.6)))+
  geom_point()+
  scale_x_log10()+
  scale_y_log10()+
  theme_bw()+
