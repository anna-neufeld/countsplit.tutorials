...
We can briefly see that, even in this small setting, we can obtain reasonable performance from 
...
We can quickly verify that negatib
...
To gain intuition for this idea, consider the following example dataset. It is similar to the dataset above, but the signal strength is stronger such that the clusters are more well-separated in log-space. 
...
The main idea is that, if we estimate three clusters on the training set and also estimate three clusters on the test set, the resulting clusterings should be very similar to one another because they are capturing true signal, not random noise.
...
On the other hand, if we estiamte either two clusters or four clusters, there will be less similarity. 
...
ggplot(data=NULL, aes(x=Xtest.known[,1]+1, y=Xtest.known[,2]+1, pch=as.factor(clusters.est.train),
                      col=as.factor(clusters.est.test)))+
  geom_point()+
  scale_x_log10()+
  scale_y_log10()+
  theme_bw()+
...
```{r}
set.seed(5)
clusters.est.train <- kmeans(log(Xtrain.known+1), centers=5)$cluster
clusters.est.test <- kmeans(log(Xtest.known+1), centers=5)$cluster
mclust::adjustedRandIndex(clusters.est.train,clusters.est.test)
...
The principle is simple: if we are estimating the correct number of clusters, our clusterings should be similar whether we estimate them on the training set or the test set. 

We use the same data that we generated above. 
...
n=1000
p=1000
lambda <- matrix(exp(rnorm(p)), nrow=n, ncol=p, byrow = TRUE)
clusters <- sample(1:3, size=n, replace=TRUE)
lambda[clusters==2,1:20] <-  lambda[clusters==2,1:20]*exp(2)
lambda[clusters==3,11:20] <-  lambda[clusters==3,11:20]*exp(2)
true.overdisps <- colMeans(lambda)
X <- sapply(1:p, function(u) rnbinom(length(lambda[,u]), mu=lambda[,u], size= true.overdisps[u]))
