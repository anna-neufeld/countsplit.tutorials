...
```{r, out.width="50%"}
ggplot(data=NULL)+
  geom_qq(aes(sample=as.numeric(pvals_graph_test_doubledip), col="Double Dipping"), distribution=stats::qunif)+
  geom_qq(distribution=stats::qunif, aes(sample=as.numeric(pvals_graph_test_countsplit), col="Count Splitting"))+
  geom_abline()+
  coord_fixed()+
  labs(col="")+theme_bw()+
...
colnames(pvals_doubledip) <- colnames(pvals_countsplit) <- paste("gene", 1:p, sep="")
...
for (i in 1:n_trials) {
  ...
  if (length(unique(Idents(cds)))>1) {
    res <-  FindMarkers(cds, ident.1 = 1, ident.2 = 2, min.pct=0, min.cells.feature=0,
                              min.cells.group=0,
                              logfc.threshold=0)
    pvals_doubledip[i, rownames(res)] <- res$p_val
  }
  ...
  if (length(unique(Idents(cds_train)))>1) {
    res <- FindMarkers(cds_train, ident.1 = 1, ident.2 = 2, min.pct=0, assay="test", min.cells.feature=0,
                                 min.cells.group=0,
                                 logfc.threshold=0)
    pvals_countsplit[i,rownames(res)] <- res$p_val
  }
}
```
...
```{r, out.width="50%"}
ggplot(data=NULL)+
  geom_qq(aes(sample=as.numeric(pvals_doubledip), col="Double dipping"), distribution=stats::qunif)+
  geom_qq(distribution=stats::qunif, aes(sample=as.numeric(pvals_countsplit), col="Count splitting"))+
  geom_abline()+
  coord_fixed()+
  labs(col="")+theme_bw()+
  xlab("Unif(0,1) Quantiles")+
