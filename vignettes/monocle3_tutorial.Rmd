...
We now deviate from the Monocle3 vignette to apply count splitting. We apply Poisson count splitting directly to the raw expression matrix. We then construct a `cell_data_set` object (used by the Monocle3 package) that contains only the training set counts. 
...
split.pcs <- countsplit(expression_matrix)
Xtrain <- split.pcs[[1]]
Xtest <- split.pcs[[2]]
...
This pipeline assumes that the data are reasonably close to following a Poisson distribution. At the bottom of this tutorial, we consider applying negative binomial count splitting. 
...
```{r, results='hide'}
cds_train <- cluster_cells(cds_train, cluster_method="louvain")
...
# Considering negative binomial count splitting

In order to apply negative binomial count splitting, the first step is to estimate overdispersion parameters. We do this using the ``sctransform`` package, which can be installed from CRAN. 

```{r}
library(sctransform)

vst <- sctransform::vst(expression_matrix)
overdisp_estimates <- vst$model_pars_fit[,1]
```
